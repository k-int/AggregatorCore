...
function onKeyDown(e) {
    ...
    if (!options.readOnly) {
        ...
        if (mod && ((e.shiftKey && code == 90) || code == 89)) {
            redo();
            return e_preventDefault(e);
        } // ctrl-shift-z, ctrl-y
    }
...
function onKeyPress(e) {
    ...
    if (code == 13) {
        if (!options.readOnly) handleEnter();
        e_preventDefault(e);
    }
...
function readInput() {
    ...
    if (rs) {
        ...
        if (sel.inverted = posLess(head, tail)) {
            from = head;
            to = tail;
        }
        else {
    ...
    if (changed) {
        ...
        updateLines({
            ...
            line: endline, 
            ch: endch
        }, splitLines(text.slice(start, end)), from, to);
...
function selectAll() {
    ...
    setSelection({
        ...
        line: endLine, 
        ch: getLine(endLine).text.length
        });
}
...
SearchCursor.prototype = {
    ...
    find: function(reverse) {
        ...
        for (;;) {
            ...
            if (reverse) {
                ...
                pos = {
                    line: pos.line-1, 
                    ch: getLine(pos.line-1).text.length
                    };
            }
            else {
    ...
    from: function() {
        if (this.atOccurrence) return copyPos(this.pos.from);
    },
    to: function() {
        if (this.atOccurrence) return copyPos(this.pos.to);
    },
